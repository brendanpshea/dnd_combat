...
def remove_greenscreen(img: Image.Image) -> Image.Image:
    """
    Convert a green screen RGB image into an RGBA image using flood-fill from the outer edges
    so internal green elements inside the icon silhouette are never keyed out.
    """
...
def slice_grid_2x2(sheet_path: str, ids: list[str], margin: int = 6):
    ...
    print(f"Loaded {sheet_path} ({width}x{height}), slicing 2x2 icon grid & keying green screen...")
