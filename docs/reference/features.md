# Features

> Generated by `npm run reference` from the data modules in `src/data/`.
> Do not edit by hand — edit the data and regenerate.

154 features — class abilities, species traits, monster traits and trinket grants,
all in one namespace because the engine resolves them the same way.

`Trigger` is how it is taken: an action, a bonus action, `free` (rider on
something else) or `passive` (always on).

| Name | ID | Trigger | Uses | Adv. dice | Notes |
| --- | --- | --- | --- | --- | --- |
| Acid Breath | `breath-acid` | action | recharge 5–6 | — | 15 ft line, 5d8 acid, DEX save |
| Acid Breath | `breath-acid-young` | action | recharge 5–6 | — | 30 ft line, 14d6 acid, DEX save |
| Action Surge | `action-surge` | free | 1/short rest | — | — |
| Adrenaline Rush | `adrenaline-rush` | bonus | proficiency/fight | — | — |
| Agonizing Blast | `agonizing-blast` | passive | — | — | read by the rules, no action of its own |
| Alert | `alert` | passive | — | — | read by the rules, no action of its own |
| Arcane Recovery | `arcane-recovery` | passive | 1/long rest | — | read by the rules, no action of its own |
| Armor of Shadows | `armor-of-shadows` | passive | — | — | read by the rules, no action of its own |
| Assassinate | `assassinate` | passive | — | — | read by the rules, no action of its own |
| Aura of Devotion | `aura-of-devotion` | passive | — | — | read by the rules, no action of its own |
| Aura of Protection | `aura-of-protection` | passive | — | — | read by the rules, no action of its own |
| Bardic Inspiration | `bardic-inspiration` | bonus | charismaMod/long rest | — | — |
| Berserker's Grip | `berserker-curse` | passive | — | — | read by the rules, no action of its own |
| Blessed Healer | `blessed-healer` | passive | — | — | read by the rules, no action of its own |
| Boots of the Winterlands | `boots-winterlands` | passive | — | — | read by the rules, no action of its own |
| Bracers of Archery | `bracers-archery` | passive | — | — | read by the rules, no action of its own |
| Bracers of Defense | `bracers-defense` | passive | — | — | read by the rules, no action of its own |
| Breath Weapon | `breath-draconic` | action | recharge 5–6 | — | 15 ft cone, 2d6 fire, DEX save |
| Brooch of Shielding | `brooch-shielding` | passive | — | — | read by the rules, no action of its own |
| Brute | `brute` | passive | — | — | read by the rules, no action of its own |
| Burrow | `burrow` | passive | — | — | read by the rules, no action of its own |
| Channel Divinity: Preserve Life | `preserve-life` | action | 1/short rest | — | — |
| Channel Divinity: Sacred Weapon | `sacred-weapon` | bonus | 1/short rest | — | — |
| Channel Divinity: Turn Undead | `turn-undead` | action | 1/short rest | — | — |
| Charge | `charge` | passive | — | — | read by the rules, no action of its own |
| Charm | `charm` | action | 1/fight | — | — |
| Cloak of Displacement | `cloak-displacement` | passive | — | — | read by the rules, no action of its own |
| Cloak of Protection | `cloak-protection` | passive | — | — | read by the rules, no action of its own |
| Cold Breath | `breath-cold` | action | recharge 5–6 | — | 15 ft cone, 5d8 cold, CON save |
| Cold Breath | `breath-cold-young` | action | recharge 5–6 | — | 15 ft cone, 9d8 cold, CON save |
| Cold Breath | `breath-cold-winter-wolf` | action | recharge 5–6 | — | 15 ft cone, 4d8 cold, CON save |
| Consume Life | `consume-life` | bonus | 1/fight | — | — |
| Countercharm | `countercharm` | passive | — | — | read by the rules, no action of its own |
| Cunning Action: Dash | `cunning-dash` | bonus | — | — | — |
| Cunning Action: Disengage | `cunning-disengage` | bonus | — | — | — |
| Cunning Action: Hide | `cunning-hide` | bonus | — | — | — |
| Cutting Words | `cutting-words` | passive | — | — | read by the rules, no action of its own |
| Danger Sense | `danger-sense` | passive | — | — | read by the rules, no action of its own |
| Dark One's Blessing | `dark-ones-blessing` | passive | — | — | read by the rules, no action of its own |
| Dark One's Own Luck | `dark-ones-own-luck` | passive | charismaMod/long rest | — | read by the rules, no action of its own |
| Deflect Attacks | `deflect-attacks` | passive | — | — | read by the rules, no action of its own |
| Disciple of Life | `disciple-of-life` | passive | — | — | read by the rules, no action of its own |
| Divine Smite | `divine-smite` | passive | — | — | read by the rules, no action of its own |
| Draconic Resilience | `draconic-resilience` | passive | — | — | read by the rules, no action of its own |
| Draconic Spells | `draconic-spells` | passive | — | — | read by the rules, no action of its own |
| Dreadful Glare | `dreadful-glare` | action | 1/fight | — | — |
| Earth Glide | `earth-glide` | passive | — | — | read by the rules, no action of its own |
| Eldritch Invocations | `eldritch-invocations` | passive | — | — | read by the rules, no action of its own |
| Elemental Affinity | `elemental-affinity` | passive | — | — | read by the rules, no action of its own |
| Empowered Strikes | `empowered-strikes` | passive | — | — | read by the rules, no action of its own |
| Engulf | `engulf` | action | — | — | — |
| Escape the Horde | `escape-the-horde` | passive | — | — | read by the rules, no action of its own |
| Evasion | `evasion` | passive | — | — | read by the rules, no action of its own |
| Expertise | `expertise` | passive | — | — | read by the rules, no action of its own |
| Extra Attack | `extra-attack` | passive | — | — | read by the rules, no action of its own |
| Fast Hands | `fast-hands` | passive | — | — | read by the rules, no action of its own |
| Fast Movement | `fast-movement` | passive | — | — | read by the rules, no action of its own |
| Fated | `fated` | passive | 3/long rest | — | read by the rules, no action of its own |
| Feral Instinct | `feral-instinct` | passive | — | — | read by the rules, no action of its own |
| Fey Charm | `fey-charm` | action | 1/fight | — | — |
| Fey Invisibility | `fey-invisibility` | action | 1/fight | — | — |
| Fiend Spells | `fiend-spells` | passive | — | — | read by the rules, no action of its own |
| Fiendish Vigor | `fiendish-vigor` | passive | — | — | read by the rules, no action of its own |
| Fighting Style: Archery | `archery` | passive | — | — | read by the rules, no action of its own |
| Fighting Style: Defense | `defense` | passive | — | — | read by the rules, no action of its own |
| Fighting Style: Dueling | `dueling` | passive | — | — | read by the rules, no action of its own |
| Fighting Style: Great Weapon Fighting | `great-weapon-fighting` | passive | — | — | read by the rules, no action of its own |
| Fighting Style: Two-Weapon Fighting | `two-weapon-fighting` | passive | — | — | read by the rules, no action of its own |
| Fire Breath | `breath-fire` | action | recharge 5–6 | — | 15 ft cone, 7d6 fire, DEX save |
| Fire Breath | `breath-fire-young` | action | recharge 5–6 | — | 15 ft cone, 16d6 fire, DEX save |
| Fire Breath | `breath-fire-chimera` | action | recharge 5–6 | — | 15 ft cone, 7d8 fire, DEX save |
| Fire Breath | `breath-fire-hound` | action | recharge 5–6 | — | 15 ft cone, 5d6 fire, DEX save |
| Fire Breath | `breath-mephit-fire` | action | recharge 6 | — | 15 ft cone, 2d4 fire, DEX save |
| Fire Form | `fire-form` | passive | — | — | read by the rules, no action of its own |
| Flurry of Blows | `flurry-of-blows` | bonus | — | — | spends its own pool |
| Font of Magic | `font-of-magic` | passive | level/long rest | — | read by the rules, no action of its own |
| Frenzy | `frenzy` | passive | — | — | read by the rules, no action of its own |
| Frost Breath | `breath-mephit-cold` | action | recharge 6 | — | 15 ft cone, 2d4 cold, DEX save |
| Gift of the Protectors | `gift-of-the-protectors` | passive | 1/short rest | — | read by the rules, no action of its own |
| Gloves of Thievery | `gloves-thievery` | passive | — | — | read by the rules, no action of its own |
| Gnomish Cunning | `gnomish-cunning` | passive | — | — | read by the rules, no action of its own |
| Healing Touch | `healing-touch` | bonus | 1/long rest | — | — |
| Heroic Inspiration | `heroic-inspiration` | free | 1/fight | — | — |
| Horn Charge | `unicorn-charge` | passive | — | — | read by the rules, no action of its own |
| Horrifying Visage | `horrifying-visage` | action | 1/fight | — | — |
| Hunter's Prey: Colossus Slayer | `colossus-slayer` | passive | — | — | read by the rules, no action of its own |
| Improved Critical (Champion) | `improved-critical` | passive | — | — | read by the rules, no action of its own |
| Innate Sorcery | `innate-sorcery` | bonus | 2/long rest | — | spends its own pool |
| Jack of All Trades | `jack-of-all-trades` | passive | — | — | read by the rules, no action of its own |
| Keen Senses | `keen-senses` | passive | — | — | read by the rules, no action of its own |
| Land's Aid | `lands-aid` | action | — | — | — |
| Lay on Hands | `lay-on-hands` | action | fiveTimesLevel/long rest | — | spends its own pool |
| Legendary Resistance | `legendary-resistance` | passive | 3/long rest | — | read by the rules, no action of its own |
| Lightning Breath | `breath-lightning` | action | recharge 5–6 | — | 30 ft line, 6d6 lightning, DEX save |
| Lightning Breath | `breath-lightning-young` | action | recharge 5–6 | — | 60 ft line, 10d10 lightning, DEX save |
| Long-Limbed | `long-limbed` | passive | — | — | read by the rules, no action of its own |
| Lucky | `lucky` | passive | — | — | read by the rules, no action of its own |
| Luring Song | `luring-song` | action | 1/fight | — | — |
| Magic Resistance | `magic-resistance` | passive | — | — | read by the rules, no action of its own |
| Magical Cunning | `magical-cunning` | action | 1/long rest | — | — |
| Magical Discoveries | `magical-discoveries` | passive | — | — | read by the rules, no action of its own |
| Mantle of Spell Resistance | `mantle-spell-resistance` | passive | — | — | read by the rules, no action of its own |
| Martial Arts | `martial-arts` | passive | — | — | read by the rules, no action of its own |
| Metamagic: Empowered Spell | `metamagic-empowered` | passive | — | — | read by the rules, no action of its own |
| Metamagic: Heightened Spell | `metamagic-heightened` | passive | — | — | read by the rules, no action of its own |
| Metamagic: Quickened Spell | `metamagic-quickened` | passive | — | — | read by the rules, no action of its own |
| Mindless Rage | `mindless-rage` | passive | — | — | read by the rules, no action of its own |
| Monk's Focus | `monks-focus` | passive | level/short rest | — | read by the rules, no action of its own |
| Natural Recovery | `natural-recovery` | passive | 1/long rest | — | read by the rules, no action of its own |
| Naturally Stealthy | `naturally-stealthy` | passive | — | — | read by the rules, no action of its own |
| Nimble Escape | `nimble-escape` | bonus | — | — | — |
| Nimble Escape: Hide | `nimble-hide` | bonus | — | — | — |
| Open Hand Technique | `open-hand-technique` | passive | — | — | read by the rules, no action of its own |
| Pack Tactics | `pack-tactics` | passive | — | — | read by the rules, no action of its own |
| Pact Magic | `pact-magic` | passive | — | — | read by the rules, no action of its own |
| Pact of the Tome | `pact-of-the-tome` | passive | — | — | read by the rules, no action of its own |
| Patient Defense | `patient-defense` | bonus | — | — | spends its own pool |
| Petrifying Breath | `petrifying-breath` | action | 1/fight | — | — |
| Poison Breath | `breath-poison` | action | recharge 5–6 | — | 15 ft cone, 6d6 poison, CON save |
| Poison Breath | `breath-poison-young` | action | recharge 5–6 | — | 15 ft cone, 12d6 poison, CON save |
| Potent Cantrip (Evoker) | `enhanced-cantrip` | passive | — | — | read by the rules, no action of its own |
| Potent Spellcasting | `potent-spellcasting` | passive | — | — | read by the rules, no action of its own |
| Prayer Bead: Bless | `prayer-bead-bless` | bonus | 1/fight | — | — |
| Rage | `rage` | bonus | rages/long rest | — | — |
| Rampage | `rampage` | passive | — | — | read by the rules, no action of its own |
| Reckless Attack | `reckless-attack` | free | — | — | — |
| Relentless Endurance | `relentless-endurance` | passive | 1/fight | — | read by the rules, no action of its own |
| Remarkable Athlete | `remarkable-athlete` | passive | — | — | read by the rules, no action of its own |
| Repelling Blast | `repelling-blast` | passive | — | — | read by the rules, no action of its own |
| Ring of Evasion | `ring-evasion` | free | 3/fight | — | read by the rules, no action of its own |
| Ring of Free Action | `free-action` | passive | — | — | read by the rules, no action of its own |
| Roving | `roving` | passive | — | — | read by the rules, no action of its own |
| Savage Attacker | `savage-attacker` | passive | — | — | read by the rules, no action of its own |
| Sculpt Spells (Evoker) | `sculpt-spells` | passive | — | — | read by the rules, no action of its own |
| Second Wind | `second-wind` | bonus | 2/short rest | — | — |
| Sneak Attack | `sneak-attack` | passive | — | 3d6 (at level 5) | read by the rules, no action of its own |
| Sorcerous Restoration | `sorcerous-restoration` | passive | 1/long rest | — | read by the rules, no action of its own |
| Sorcery Incarnate | `sorcery-incarnate` | passive | — | — | read by the rules, no action of its own |
| Steady Aim | `steady-aim` | bonus | — | — | — |
| Step of the Wind | `step-of-the-wind` | bonus | — | — | spends its own pool |
| Stunning Strike | `stunning-strike` | bonus | — | — | spends its own pool |
| Trampling Charge | `trampling-charge` | passive | — | — | read by the rules, no action of its own |
| Trance | `trance` | passive | — | — | read by the rules, no action of its own |
| Unarmored Defense | `unarmored-defense` | passive | — | — | read by the rules, no action of its own |
| Unarmored Defense | `monk-defense` | passive | — | — | read by the rules, no action of its own |
| Uncanny Dodge | `uncanny-dodge` | passive | — | — | read by the rules, no action of its own |
| Undead Fortitude | `undead-fortitude` | passive | — | — | read by the rules, no action of its own |
| Wail | `wail` | action | 1/fight | — | — |
| Wand of the War Mage +1 | `war-mage-1` | passive | — | — | read by the rules, no action of its own |
| Wand of the War Mage +2 | `war-mage-2` | passive | — | — | read by the rules, no action of its own |
| Wave of Terror | `wave-of-terror` | action | 3/fight | — | — |
| Whelm | `whelm` | action | 1/fight | — | — |
| Whirlwind | `whirlwind` | action | 1/fight | — | — |
| Wild Shape | `wild-shape` | bonus | 2/short rest | — | spends its own pool |

## Conditions

What the engine can put on a combatant, and what the log calls it.

| ID | Log name |
| --- | --- |
| `aiming` | taking steady aim |
| `baned` | baned |
| `blessed` | blessed |
| `blinded` | blinded |
| `bonded` | warded by a bond |
| `burning` | burning |
| `charmed` | charmed |
| `commanded` | commanded |
| `confused` | confused |
| `cursed` | cursed |
| `deathWarded` | warded against death |
| `dodging` | dodging |
| `energyWarded` | warded against an element |
| `fleeing` | fleeing |
| `frightened` | frightened |
| `grappled` | grappled |
| `guided` | marked by light |
| `hasted` | hasted |
| `hexed` | hexed |
| `hidden` | hidden |
| `incapacitated` | incapacitated |
| `innateSorcery` | burning with innate sorcery |
| `inspired` | inspired |
| `inspiring` | holding an inspiration die |
| `lured` | spellbound |
| `marked` | marked by the hunter |
| `noReactions` | unable to react |
| `outlined` | outlined in light |
| `paralyzed` | paralyzed |
| `poisoned` | poisoned |
| `prone` | prone |
| `protected` | protected from evil and good |
| `raging` | raging |
| `reckless` | attacking recklessly |
| `restrained` | restrained |
| `sacredWeapon` | wielding a sacred weapon |
| `sanctuary` | under sanctuary |
| `sapped` | sapped |
| `shielded` | shielded |
| `shillelagh` | wielding a shillelagh |
| `silenced` | silenced |
| `slowed` | slowed |
| `smiting` | ready to smite |
| `stunned` | stunned |
| `unbound` | moving freely |
| `unconscious` | unconscious |
| `veiled` | invisible |
| `vexed` | vexed |
| `warded` | warded |
| `wounded` | wounded — cannot heal |
